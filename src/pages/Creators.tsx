  ...
  const fetchCreators = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("creators").select("*").order("total_ingresos_mxn", {
        ascending: false
      }).limit(50);
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 md:px-6 py-8 max-w-7xl">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {SORT_OPTIONS.map(option => <button key={option.value} onClick={() => setSortBy(option.value)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${sortBy === option.value ? "bg-primary text-primary-foreground shadow-lg" : "bg-secondary/50 text-secondary-foreground hover:bg-secondary border border-border"}`}>
              {option.label}
            </button>)}
        ...
        {/* Creator Cards */}
        {sortedCreators.length === 0 ? <Card className="p-12 text-center">
            ...
            return <Card key={creator.id} className="overflow-hidden transition-all duration-300 hover:translate-y-[-3px] hover:shadow-xl bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl shadow-lg">
                    <CardContent className="p-5">
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        <div className="relative">
                          ...
                          {isTop5(globalIndex) && <div className="absolute -top-1 -right-1 bg-gradient-to-r from-orange-500 to-red-500 rounded-full p-1.5 shadow-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <Badge variant="outline" className={`mt-1.5 text-xs font-bold ${isTop5(globalIndex) ? "bg-gradient-to-r from-orange-500/10 to-red-500/10 border-orange-500/30 text-orange-600" : "bg-primary/10 border-primary/30 text-primary"}`}>
                            #{ranking}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-1.5 mb-4">
                        <div className="p-2 rounded-xl bg-purple-500/10 border border-purple-500/20 text-center">
                          ...
                          <p className="text-xs font-bold text-purple-600">
                            {creator.total_live_count && creator.total_live_count > 0 ? formatNumber(creator.total_live_count) : "—"}
                          </p>
                        </div>
                      ...
                      {/* CTA Button */}
                      {tiktokUrl && <Button size="default" className="w-full font-medium text-sm bg-primary hover:bg-primary/90 text-primary-foreground shadow-md transition-all duration-200 hover:shadow-lg rounded-xl" onClick={() => window.open(tiktokUrl, '_blank')}>
            ...
            {totalPages > 1 && <div className="flex items-center justify-center gap-2 mt-8">
                ...
                <div className="flex gap-1">
                  {getPageNumbers().map((page, index) => typeof page === 'number' ? <Button key={index} variant={currentPage === page ? "default" : "outline"} size="sm" onClick={() => goToPage(page)} className="w-9 h-9">
                        {page}
                      </Button> : <span key={index} className="px-2 self-center text-muted-foreground">
                        {page}
                      </span>)}
                </div>
