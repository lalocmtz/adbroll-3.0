  ...
  try {
    ...
    const processedEvents = await Promise.all(events.map(async (event) => {
      ...
      if (event.user_data?.phone) {
        userData.ph = [await hashData(event.user_data.phone)];
      }
      ...
      // Add Facebook click and browser IDs if available
    ...
    console.log('Sending events to Meta Conversions API:', JSON.stringify(processedEvents, null, 2));
